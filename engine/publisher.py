...
def publish(
    ...
    *,
    ...
    if mode == "morning":
        import os
        force = os.getenv("GITHUB_EVENT_NAME", "") in ("workflow_dispatch", "push")
        if has_locked and force and picks:
            logger.info(f"Morning RE-PUBLISH ({os.getenv('GITHUB_EVENT_NAME')}): replacing {len(existing_today_main)} picks with {len(picks)} new picks")
